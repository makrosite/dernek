@php

use App\Models\Isbirlikleri;
 $firma = Isbirlikleri::all();


    // Kayma hızını ayarlamak için bir değişken oluşturun (1-10 arası, daha yüksek değer daha hızlı kayar)
    $kaymaHizi = 15; // Buradan hız ayarlanabilir
@endphp
<div style="background-color: #f8f8f8; padding: 20px 0; border-top: 1px solid #e0e0e0; border-bottom: 1px solid #e0e0e0; margin-top: 0; margin-bottom: 30px;">
    <div class="container-fluid" style="padding-left:120px; padding-right:120px;">
        {{-- Modern Başlık Bölümü --}}
        <div style="text-align: center; margin-bottom: 40px;"> <!-- Başlık ve logolar arası boşluk -->
            <h2 style="font-size: 36px; color: #0d1e50; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; display: inline-block; position: relative; padding-bottom: 10px;">
                İş Ortaklarımız
                <span style="content: ''; display: block; width: 60px; height: 3px; background-color: #0d1e50; margin: 10px auto 0; border-radius: 2px;"></span>
            </h2>
        </div>
        {{-- Kayan Logolar Bölümü --}}
        <marquee behavior="scroll" direction="left" scrollamount="{{ $kaymaHizi }}" style="height: 100px; line-height: 100px;">

@foreach ($firma as $f )


            {{-- Örnek logo görselleri, daha sonra veritabanından çekilebilir --}}
            <img src="{{ asset('isbirlikleri/' . $f->resim) }}" alt="Müşteri Logosu 21" style="height: 70px; vertical-align: middle; margin: 0 40px; opacity: 0.8;">
       @endforeach
        </marquee>
    </div>
</div>
